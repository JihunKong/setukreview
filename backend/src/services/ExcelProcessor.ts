  ...
  async processFile(buffer: Buffer, fileName: string): Promise<ExcelData> {
    try {
      ...
      workbook.SheetNames.forEach(sheetName => {
        ...
        // Convert worksheet to JSON format while preserving cell positions
      ...
      console.log(`📋 Sheets found: ${Object.keys(excelData.sheets).join(', ')}`);
      
      // Log cell count for each sheet
      Object.entries(excelData.sheets).forEach(([sheetName, sheet]) => {
        const cellCount = sheet.data.flat().filter(cell => cell !== '').length;
        console.log(`  - ${sheetName}: ${cellCount} non-empty cells`);
      });
