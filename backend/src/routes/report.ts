...
router.post('/batch/download', [
  ...
  console.log(`🔍 DEBUG: Looking for validation IDs: ${JSON.stringify(validationIds)}`);
  const results = validationIds
    .map(id => {
      const result = ValidationService.getResult(id);
      console.log(`🔍 DEBUG: ValidationService.getResult(${id}) = ${!!result ? 'FOUND' : 'NOT FOUND'}`);
      return result;
    })
    .filter((result): result is NonNullable<typeof result> => result !== undefined);

  console.log(`🔍 DEBUG: Found ${results.length} validation results out of ${validationIds.length} requested`);
