...
export class ReportGenerator {
  ...
  async generateBatchExcelReport(results: ValidationResult[]): Promise<Buffer> {
    try {
      ...
      // Individual file sheets (summary only to avoid excessive size)
      results.forEach((result, index) => {
        if (index < 20) { // Limit to first 20 files to avoid Excel sheet limit
          const fileData = [
            ['파일명', result.fileName],
            ['검증 ID', result.id],
            ['상태', result.status],
          ...
          const sheetName = `${index + 1}_${result.fileName.slice(0, 20)}`;
